  ...
  module Billing #:nodoc:
    class VisanetPeruGateway < Gateway
      ...
      def capture(authorization, options={})
        params = {}
        options[:id_unico] = parse_auth(authorization)[1]
      ...
      def refund(amount, authorization, options={})
        ...
        return response if response.success? || parse_auth(authorization).length == 1
      ...
      def add_auth_order_id(params, authorization, options)
        purchase_number, _ = parse_auth(authorization)
      ...
      def prepare_refund_data(params, authorization, options)
        ...
        _ , transaction_id = parse_auth(authorization)
      ...
      def message_from(response, options, action)
        if empty?(response["errorMessage"]) || response["errorMessage"] == "[ ]"
          "#{response["data"]["DSC_COD_ACCION"]}, #{options[:error_message]}" if action == 'refund'
