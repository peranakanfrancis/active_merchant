...
class RemoteMundipaggTest < Test::Unit::TestCase
  def setup
    ...
    @voucher = credit_card('6033890101091447', verification_value: '377', year: 2021, month: 9)
    @options = {
  ...
  def test_successful_purchase_with_voucher
    @options.update({holder_document: '93095135270', voucher: true})
  ...
  def test_successful_create_customer
    @options.update(name: 'Sideshow Bob')
    response = @gateway.create_customer(@options)
    assert !response.params["id"].blank?
  end

  def test_successful_store_and_purchase
    customer = @gateway.create_customer(options = {name: 'Bob Belcher'})
  ...
  def test_successful_store_and_purchase_no_customer
    store = @gateway.store(@credit_card, @options)
    assert_success store
